...
- **A reset routine is granted no tools at all.** `allowed_tools` is `[]`, from `window_optimizer.ping_content.allowed_tools()`, never composed ad hoc in a command file. Every prompt in the pool is self-contained — nothing fetches, reads, or reaches outside itself (`adr/0010-fixed-safe-prompt-pool.md`). Never grant `Bash`, `WebFetch`, a repo checkout, or an MCP connector to a reset routine, and never add a prompt that would need one, without a new ADR superseding that one.
