...
- **`allowed_tools` is not what was requested** → stop everything and report the exact difference. Never repair a tool grant by guessing what it should be. (Since `adr/0010-fixed-safe-prompt-pool.md` the requested grant is always `[]`, so this check is now simply: anything non-empty is a divergence.)
