...
2. **Pings that can do something real, not just a no-op** — each ping defaults to a plain keep-alive, but you can optionally point one (e.g. a midday slot, if that's usually your coding time) at a public GitHub repo to check its most recently updated open issues instead — still one cheap `WebFetch` call on the `haiku` model, not "real work." See `adr/0002-useful-ping-content.md` for what's supported today and what isn't yet (email/calendar checks aren't built — disclosed, not silently skipped).
